 ...
 *
 * @param {number} inventory_slot Index of item to store.
 * @param {string} storage Bank pack (e.g. `"items0"`)
 * @param {number} [storage_slot=-1] Pack index to store/swap item in (default: first available).
 */
export function bank_store(inventory_slot, storage, storage_slot) {
	storage_slot = storage_slot >= 0 ? storage_slot : -1;

	if (!character.bank) {
		game_log("Not inside the bank");
		return;
	}

	if (!character.items[inventory_slot]) {
		game_log("No item in that spot");
		return;
	}

	console.debug(`${inventory_slot} => ${storage}:${storage_slot}`);
	parent.socket.emit('bank', { operation: 'swap', pack: storage, str: storage_slot, inv: inventory_slot });
}
 ...
 *
 * @param {string} storage Bank pack (e.g. `"items0"`)
 * @param {number} storage_slot Index of item to retrieve.
 * @param {number} [inventory_slot=-1] Inventory index to store/swap items.
 */
export function bank_retrieve(storage, storage_slot, inventory_slot) {
	inventory_slot = inventory_slot >= 0 ? inventory_slot : -1;
	...
	if (!character.bank[storage][storage_slot]) {
		game_log("No item in that spot");
		return;
	}

	console.debug(`${storage}:${storage_slot} => ${inventory_slot}`);
	parent.socket.emit('bank', { operation: 'swap', pack: storage, str: storage_slot, inv: inventory_slot });
}
